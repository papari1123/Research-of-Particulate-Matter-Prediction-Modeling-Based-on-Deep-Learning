...
def build_model(PM_xy_hash_table, train_lstm, train_conv, train_china, val_lstm, val_conv, val_china, outputname, n_input_timestep, n_output_timestep, 
	...
	lstm_encoder = Conv3D(filters=filter_n*2 , kernel_size=(3,1,1), kernel_regularizer = g_regular1)(lstm_1)
	lstm_encoder = Activation(selu)(lstm_encoder) 
	lstm_encoder = Conv3D(filters=filter_n*2, kernel_size=(3,1,1), kernel_regularizer = g_regular1)(lstm_encoder)
	...
	mix_encoder = ConvLSTM2D(filters=mix_encoder_filter, padding= 'same', dropout= g_dropout, kernel_size=(1,1), activation='relu', kernel_regularizer = g_regular2)(mix_encoder)   
...
#
	adam_ = optimizers.Adam(lr=0.001, beta_1=0.9, beta_2=0.999, epsilon=0.0000001, decay=0, amsgrad=True)
	if(main.is_One_Station is True): 
