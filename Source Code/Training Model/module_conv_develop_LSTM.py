...
def to_supervised_china(train_china, n_input_timestep, n_output_timestep, PM_station_num, PM_xy_hash_table, matrix_x,matrix_y ):
	...
	for train_id in range(len(train_china[0])):
		...
		if out_end < len(train_china[0]):
			...
			xbeijing_input = train_china[0][in_start:in_end, :,:,:]
...
def build_model(PM_xy_hash_table, train_lstm, train_conv, train_china, val_lstm, val_conv, val_china, outputname, n_input_timestep, n_output_timestep, 
	...
	if(g_is_china != 5 and g_is_china != 6 and g_is_china != 7 and  g_is_china != 20 and g_is_china > 0) :  # 4차원
		train_x3 = to_supervised_china(train_china, n_input_timestep, n_output_timestep, PM_station_num, PM_xy_hash_table, matrix_x, matrix_y)
		val_x3 =  to_supervised_china(val_china, n_input_timestep, n_output_timestep, PM_station_num, PM_xy_hash_table, matrix_x, matrix_y)
	...
	beijing_input = Input(shape=(n_input_timestep, matrix_x, matrix_y, train_china[0].shape[3]))

	if(g_is_china == 5 or g_is_china == 6 or g_is_china == 7 or g_is_china == 20) :   # 3차원
	...
	lstm_1 = lstm_input
	if(g_version == 0) :
		lstm_encoder = GRU_recurrent.ConvGRU2D(filters=filter_n*2, padding= 'same', dropout= g_dropout , kernel_size=(5,5), return_sequences = True, activation='relu', kernel_regularizer = g_regular1)(lstm_1)
		mix_encoder = lstm_encoder
	elif(g_is_china != 14) :
		conv_encoder = GRU_recurrent.ConvGRU2D(filters=filter_n, padding= 'same',dropout= g_dropout , kernel_size=(kernel_size,kernel_size), return_sequences = True, activation='relu', kernel_regularizer = g_regular1)(conv_1)
		lstm_encoder = GRU_recurrent.ConvGRU2D(filters=filter_n*2, padding= 'same', dropout= g_dropout , kernel_size=(5,5), return_sequences = True, activation='relu', kernel_regularizer = g_regular1)(lstm_1)
		mix_encoder = concatenate([conv_encoder, lstm_encoder])   
	else : 
		conv_encoder = GRU_recurrent.ConvGRU2D(filters=filter_n, padding= 'same',dropout= g_dropout , kernel_size=(kernel_size,kernel_size), return_sequences = True, activation='relu', kernel_regularizer = g_regular1)(conv_1)
		lstm_encoder = GRU_recurrent.ConvGRU2D(filters=filter_n*2, padding= 'same', dropout= g_dropout , kernel_size=(5,5), return_sequences = True, activation='relu', kernel_regularizer = g_regular1)(lstm_1)
		china_encoder = GRU_recurrent.ConvGRU2D(filters=filter_n, padding= 'same',dropout= g_dropout , kernel_size=(kernel_size,kernel_size), return_sequences = True, activation='relu', kernel_regularizer = g_regular1)(beijing_input)
		mix_encoder = concatenate([conv_encoder, lstm_encoder, china_encoder])   
       
	mix_encoder_filter =  filter_n*2
    
	mix_encoder = GRU_recurrent.ConvGRU2D(filters=mix_encoder_filter, padding= 'same', dropout= g_dropout, kernel_size=(kernel_size,kernel_size), activation='relu', kernel_regularizer = g_regular2)(mix_encoder)

	if(g_is_china == 20) :
		b_input = LocallyConnected2D(filters = filter_n, kernel_size = (1,1), activation='relu', kernel_regularizer = g_regular1)(beijing_input)
		mix_encoder = concatenate([mix_encoder, b_input])
		mix_encoder_filter =  filter_n*3
	...
	mix_decoder = GRU_recurrent.ConvGRU2D(filters=filter_n*2, padding= 'same', dropout= g_dropout , kernel_size=(kernel_size,kernel_size),
                              return_sequences = False, activation='relu', kernel_regularizer = g_regular2)(mix_decoder)   
        
   
	adam_ = optimizers.Adam(lr=0.00075, beta_1=0.9, beta_2=0.999, epsilon=0.0000001, decay=0, amsgrad=True)
	if(main.is_One_Station is True): 
		...
		if(g_is_china == 5) :
				b_input = LocallyConnected2D(filters = filter_n, kernel_size = (1,1), activation='relu', kernel_regularizer = g_regular1, name="china_connected")(beijing_input)
            
				mix_decoder = concatenate([mix_decoder, b_input])
		elif(g_is_china == 6) :
				b_layer = Conv2D(filters = filter_n, kernel_size = (1,1), activation='relu', kernel_regularizer = g_regular1, name = "china_connected")(beijing_input)
		...
		if(g_is_china>0) :
				model = Model(inputs=[conv_input, lstm_input, beijing_input], outputs=output) 
		else :
	...
	for layer in model.layers:
		if layer.name == 'china_connected':
			print('trainable - false')
			layer.trainable = False#	model.load_weights(g_checkpoint_path)
	...
	if(main.isSaved is False) : 
				if(g_is_china>0) :
						hist = model.fit([train_x1, train_x2, train_x3], train_y, epochs=epochs, batch_size=batch_size,  callbacks=   callbacks_list, validation_data=([val_x1, val_x2, val_x3], val_y), verbose=verbose)
				else :
	...
	for layer in model.layers:
		if layer.name == 'china_connected':
			...
			layer.trainable = True#	model.load_weights(g_checkpoint_path)
			model.compile(optimizer= adam_, loss= 'mse', metrics=['mae'])
			model.summary()      
			model.fit([train_x1, train_x2, train_x3], train_y, epochs=epochs, batch_size=batch_size,  callbacks=   callbacks_list, validation_data=([val_x1, val_x2, val_x3], val_y), verbose=verbose)        
...
# make a forecast
def forecast(model, history_lstm, history_conv, history_beijing  ,  map_normal_max, matrix_x, matrix_y):
	# flatten data
	...
	if(g_is_china == 5 or g_is_china == 6 or g_is_china == 7 or g_is_china == 20) :
		input_beijing = data_beijing[-1, :]    
		input_beijing = input_beijing.reshape((1, matrix_x, matrix_y, input_beijing.shape[2]))

	elif(g_is_china != 0) : 
		input_beijing = data_beijing[-main.input_timestep:, :]    
		input_beijing = input_beijing.reshape((1, main.input_timestep, matrix_x, matrix_y, input_beijing.shape[3]))
	...
	if(g_is_china>0)      :
		yhat = model.predict([input_conv, input_lstm, input_beijing],verbose=0)  
	else : 
...
#먼지 데이터    
    ...
    """
    # 3시간단위로 추가..
    china_dataset_3 =  np.zeros((baekryongdo_dataset.shape[0]))
    china_dataset_3[0] = china_dataset[0,china_col]
    china_dataset_3[1] = (china_dataset[0,china_col]+china_dataset[1,china_col])/2
    for i in range (2, china_dataset_3.shape[0]) :
        china_dataset_3[i] = (china_dataset[i-2,china_col] +  china_dataset[i-1,china_col] + china_dataset[i,china_col])/3
    
    # 중국 데이터 china_timestep 추가

    start = baekryongdo_size+baekryongdo_timestep*3
    
    china_group_mem = 3
    china_timestep_dummy = china_timestep*china_group_mem

    
    combine_map_merged[0,:,:,start:start+china_timestep] = china_dataset_3[0] # 0먼저 채워넣기
    
    for time in range (1,china_timestep_dummy):  # 그 뒤..
        combine_map_merged[time,:,:,start:start+china_timestep-int(time/3)] = china_dataset_3[0]
        for step in range (china_timestep-int(time/3), china_timestep) :
              combine_map_merged[time,:,:,start + step] = china_dataset_3[int(time/3)]

    for time in range (china_timestep_dummy, china_dataset.shape[0]) :
        for step in range (0, china_timestep) :
            combine_map_merged[time,:,:,start+step] = china_dataset_3[time-china_timestep_dummy+step*3]
...
def evaluate_model(version, dropout, regular1, regular2, is_china, PM_xy_hash_table, n_filter, is_terrain, is_skip, kernel_size, outputname, conv_data, lstm_data,
    ...
    #### China
#	beijing_china_data = get_china_data('combine_IDW_china_99.npy',1)
...
#     history is a list of weekly data
	...
	for i in range(len(test_conv)-main.output_timestep+1):
		...
		yhat_sequence = forecast(model, history_lstm, history_conv, history_beijing, map_normal_max, matrix_x, matrix_y)
