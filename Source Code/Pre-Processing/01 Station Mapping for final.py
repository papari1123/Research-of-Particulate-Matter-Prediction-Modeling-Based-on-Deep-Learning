...
MAPPING = True
if(MAPPING) :
    dataset = read_csv('./data/미세먼지 측정소 주소_final.csv', header=0, index_col = 0, engine='python') 
...
"""    
DATA_TRANS1 = True   
feature = ['측정소코드','측정일시','PM10','SO2','NO2']
...
if(DATA_TRANS1) :   
    ...
    dataset.to_csv('./output/2014-2018 all_PM10.csv',header=True, index=True)
...
extractFeature = ['PM10','SO2','NO2']

DATA_TRANS2 = True   

if(DATA_TRANS2) :
    dataset = read_csv('./output/2014-2018 all_PM10.csv', header=0, index_col = 1, engine='python') 
    for fea in range(len(extractFeature)): #feature별 생성
        ...
        pmSet.to_csv('./output/2014-2018 all_'+extractFeature[fea]+'.csv',header=True, index=True)
